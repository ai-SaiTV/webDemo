<template>
  ...
  <div class="flex h-screen">
    <!-- 侧边导航栏 -->
    <div class="w-28 bg-gray-200 p-4 flex flex-col items-center justify-between">
      <div class="space-y-4">
        <button v-for="n in 5" :key="n" @click="setStep(n - 1)"
          class="btn rounded-full w-24 h-12 text-white bg-blue-500">
          {{ `步骤${n - 1}` }}
        </button>
      </div>
    </div>
    <!-- <div class="flex flex-col items-center justify-center h-screen text-center bg-gray-100"></div> -->
    <div class="flex-grow p-6 bg-gray-100">
      <!-- 步骤 0: 导入数据库 -->
      <div v-if="step === 0" class="step p-6 bg-white rounded-lg shadow-md w-3/4">
        <h2 class="text-2xl font-bold mb-4">步骤 0: 导入数据库（可选）</h2>
        <p class="mb-4">请导入pdf文件，生成数据库。</p>
        <div v-if="uploadedFiles.length > 0" class="mt-4">
          <h3 class="text-lg font-semibold">已上传的PDF文件：</h3>
      ...
      <div v-if="step === 4" class="step p-6 bg-white rounded-lg shadow-md w-3/4">
        ...
        <button @click="previousStep" class="btn">上一步</button>
        <button @click="produceExercises" class="btn">生成练习题</button>
